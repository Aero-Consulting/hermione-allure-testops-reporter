...
module.exports = class AllureHermioneReporter {
		...
		if (this.runningTest === null) {
			...
			if (test.hermioneCtx.assertViewResults.hasFails()) {
				...
				failedTests.forEach(async (failedTest) => {
					const stateName = failedTest.stateName;

					const diffPath = await this.#saveDiff(failedTest);
	...
	async createDiffImg (failedTest) {
		const createDiff = (opts) => {
			return new Promise((resolve, reject) =>
				looksSame.createDiff(opts, (error, buffer) => {
		...
		// const { diffColor: highlightColor, ...otherOpts } = failedTest.diffOpts;
		const diffOptions = failedTest.diffOpts;
		const actualImgBuffer = fs.readFileSync(failedTest.currImg.path);
		const refImgBuffer = fs.readFileSync(failedTest.refImg.path);

		diffOptions.current = actualImgBuffer;
		diffOptions.reference = refImgBuffer;

		const buffer = await createDiff(diffOptions);

		const diffImgInst = new Image(buffer);

		temp.init();
		const diffPngPath = temp.path({ suffix: '.png' });
		const diffImg = {
			path: diffPngPath,
	...
	async #saveDiff (failedTest) {
		const currPath = failedTest.currImg.path;
		const refPath = failedTest.refImg.path;
		...
		this.createDiffFile(currPath, refPath, tempPath);
	}
};
