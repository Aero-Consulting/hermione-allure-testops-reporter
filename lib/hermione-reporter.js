  ...
  failTestCase (test, error) {
    if (this.runningTest === null) {
      ...
      if (test.hermioneCtx.assertViewResults.hasFails()) {
        ...
        if (test.hermioneCtx.assertMobileViewResults) {
          const diffImg =
            test.hermioneCtx.assertMobileViewResults._results[0].diffImg

          this.writeAttachment('ImageDiff', diffImg.path, 'image/png')
        }
